...
int main() {
  trm::Trm<int> shared_int(0);

  std::thread t1([shared_int]() mutable {
    shared_int.withLock([](int &value) {
      value += 10;
      std::cout << "Thread 1: " << value << std::endl;
    });
  });

  std::thread t2([shared_int]() mutable {
    shared_int.withLock([](int &value) {
      value += 20;
  ...
  std::thread t3([shared_int]() {
    shared_int.withConstLock([](const int &value) {
